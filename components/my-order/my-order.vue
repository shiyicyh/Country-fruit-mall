<template>
  <view class="order" >
    <view @click.capture="gotoOrderDetail()" >
    <view class="title">
    ...
    <my-order-button :status="order.status" @orderButton.stop="orderOperation()" @goto.stop="gotoPage()"></my-order-button>
  </view>
</template>
    ...
    methods:{
      ...
      getGoodsInfo(){
        for(let i=0;i<this.order.buy_goods.length;i++){
          ...
          .field('_id,goods_thumb,name,goods_price,standard')
        	.get()
        	.then((res)=>{
            console.log("订单商品信息")
            console.log(res)
      ...
      gotoPage(s){
        if(s=='comment'){
          ...
          uni.navigateTo({
            url: '/subpkg/goods_comment/goods_comment?orderId='+this.order._id+''
          })
        }
      },
      gotoOrderDetail(){
